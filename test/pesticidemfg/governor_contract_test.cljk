(ns pesticidemfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the formulation/mixing or filling/
  packaging line directly... does NOT decide or grant a pesticide
  registration or product-label approval') implemented faithfully. The
  single invariant under test:

    PesticideAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Pesticide & Agrochemical
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :herbicide}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :herbicide (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest registration-decision-is-held-and-permanently-blocked
  (testing "a proposal that sets :decide-registration? true -> HOLD, PERMANENT, never reaches request-approval -- deciding a pesticide registration/label approval is exclusively a regulatory authority's call"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:registration-decision-blocked} (-> (store/ledger db) last :basis)))
      (is (not (:decide-registration? (store/batch db "batch-001")))
          "the registration-decision attempt never lands in the SSoT"))))
...
(deftest invalid-product-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-type :unobtainium-chemical}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-chemical (:product-type (store/batch db "batch-001"))) "fabricated product-type never lands in the SSoT")))

(deftest invalid-active-ingredient-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:active-ingredient-pct -5.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-active-ingredient} (-> (store/ledger db) last :basis)))
    (is (not= -5.0 (:active-ingredient-pct (store/batch db "batch-001"))) "fabricated active-ingredient concentration never lands in the SSoT")))

(deftest active-ingredient-exceeds-label-limit-is-held
  (testing "a batch patch whose own declared active-ingredient concentration exceeds its product type's own regulatory label ceiling -> HOLD"
    (let [[db actor] (fresh)
          res (exec-op actor "t11c" {:op :log-production-batch :effect :propose :subject "batch-001"
                                     :patch {:active-ingredient-pct 95.0}} coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:active-ingredient-exceeds-label-limit} (-> (store/ledger db) last :basis)))
      (is (= 41.0 (:active-ingredient-pct (store/batch db "batch-001")))
          "the over-limit active-ingredient patch never lands in the SSoT -- batch-001 keeps its seeded active-ingredient concentration"))))

(deftest active-ingredient-within-limit-auto-commits
  (testing "a batch patch whose own declared active-ingredient concentration stays within its product type's own regulatory label ceiling -> auto-commits cleanly"
    (let [[db actor] (fresh)
          res (exec-op actor "t11d" {:op :log-production-batch :effect :propose :subject "batch-001"
                                     :patch {:active-ingredient-pct 55.0}} coordinator)]
      (is (= :commit (get-in res [:state :disposition])))
      (is (= 55.0 (:active-ingredient-pct (store/batch db "batch-001")))))))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "blender-001" :severity :moderate
                                            :description "active-ingredient exposure risk rising near formulation equipment"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "blender-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
