(ns pesticidemfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [pesticidemfg.registry :as r]))
...
(deftest known-product-types-are-valid
  (doseq [g [:herbicide :insecticide :fungicide :rodenticide :nematicide
...
(deftest fabricated-product-type-is-invalid
  (is (not (r/product-type-valid? :unobtainium-chemical)))
  (is (not (r/product-type-valid? nil))))

;; ----------------------------- active-ingredient-valid? -----------------------------

(deftest typical-active-ingredient-concentration-is-valid
  (is (r/active-ingredient-valid? 41.0))
  (is (r/active-ingredient-valid? 0.0))
  (is (r/active-ingredient-valid? 10.0))
  (is (r/active-ingredient-valid? 100.0)))

(deftest negative-active-ingredient-concentration-is-invalid
  (is (not (r/active-ingredient-valid? -5.0))))

(deftest excessive-active-ingredient-concentration-is-invalid
  (is (not (r/active-ingredient-valid? 150.0)))
  (is (not (r/active-ingredient-valid? 100.01))))

(deftest non-numeric-or-missing-active-ingredient-concentration-is-invalid
  (is (not (r/active-ingredient-valid? nil)))
  (is (not (r/active-ingredient-valid? "41.0"))))

;; ----------------------------- active-ingredient-label-limit-for / active-ingredient-exceeds-label-limit? -----------------------------

(deftest every-valid-product-type-has-a-label-limit
  (doseq [pt r/valid-product-types]
    (is (some? (r/active-ingredient-label-limit-for pt))
        (str pt " must have an active-ingredient-concentration label ceiling"))))

(deftest active-ingredient-within-limit-does-not-exceed
  (is (false? (r/active-ingredient-exceeds-label-limit? :herbicide 41.0))))

(deftest active-ingredient-exactly-at-limit-does-not-exceed
  (is (false? (r/active-ingredient-exceeds-label-limit? :herbicide 62.0))
      "exactly at the ceiling is not over, only strictly beyond"))

(deftest active-ingredient-over-limit-exceeds
  (is (true? (r/active-ingredient-exceeds-label-limit? :herbicide 95.0))))

(deftest missing-active-ingredient-is-never-flagged-exceeded
  (is (false? (r/active-ingredient-exceeds-label-limit? :herbicide nil))))

(deftest unknown-product-type-with-no-ceiling-is-never-flagged-exceeded-here
  (is (false? (r/active-ingredient-exceeds-label-limit? :unobtainium-chemical 999.0))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "blender-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "blender-001" 1)
