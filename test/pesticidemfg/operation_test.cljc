(ns pesticidemfg.operation-test
  "Smoke tests for the compiled PesticideOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `pesticidemfg.governor-contract-test`; the Store contract in
  `pesticidemfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "PesticideOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "blender-001" :severity :moderate :description "有効成分曝露リスク上昇"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
