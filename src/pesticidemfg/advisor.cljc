(ns pesticidemfg.advisor
  "PesticideAdvisor -- the *contained intelligence node* for the
  pesticide and other agrochemical product plant-operations
  coordination actor.

  It normalizes production-batch patches (product-type/weight/active-
  ingredient-concentration data), drafts a formulation/mixing-
  equipment (blending tank, high-shear mixer) or filling/packaging-
  line maintenance scheduling proposal against a piece of equipment,
  drafts a safety-concern flag, and drafts an outbound shipment
  ...
  a real formulation/mixing-line actuation, freight dispatch, or
  pesticide-registration/label-approval decision. Every output is
  censored downstream by `pesticidemfg.governor` before anything
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `pesticidemfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct formulation/
                                 ; mixing-line-control effect and NEVER
                                 ; a pesticide-registration/label-
                                 ; approval decision
  ...
  `pesticidemfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  weight, active-ingredient-concentration, or verification status.
...
(defn- schedule-maintenance
  "Draft a formulation/mixing-equipment (blending tank, high-shear
  mixer) or filling/packaging-line maintenance-window scheduling
  ...
  `pesticidemfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a chemical-hazard (toxicity/exposure-risk)/environmental-
  contamination/equipment-safety concern. ALWAYS `:stake
  ...
  `pesticidemfg.phase`: no phase ever adds this op to a phase's
  `:auto` set; `pesticidemfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound pesticide/agrochemical-product shipment
  coordination proposal against a production batch. The advisor
  passes through the caller's own claimed weight -- it does NOT
  invent one, and `pesticidemfg.governor` NEVER trusts it: it
  independently recomputes whether the batch's own cumulative-shipped
  weight plus this claim would exceed the batch's own recorded weight
...
(def ^:private system-prompt
  (str "あなたは農薬・その他農業化学品工場プラント運用コーディネーターの"
       ...
       "配合/混合タンク・高せん断ミキサー・充填/包装ライン等の設備の直接操作"
       "(actuate)を絶対に提案してはいけません(この actor は提案のみを行い、"
       "実行は一切行いません)。出荷量を偽って報告してはいけません。"
       "農薬登録またはラベル承認の可否を判断・付与してはいけません"
       "(この actor は規制当局ではありません)。有効成分濃度が"
       "ラベル上限を超過するバッチを適合と偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `pesticidemfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :pesticidemfg-advisor-proposal
   :op         (:op request)
