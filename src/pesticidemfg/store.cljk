(ns pesticidemfg.store
  "SSoT for the pesticide and other agrochemical product plant-
  operations coordination actor, behind a `Store` protocol so the
  backend is a swap, not a rewrite -- the same seam every
  `cloud-itonami-isic-*` actor in this fleet uses.

  Scope note: like its closest chemical-process-plant analog
  (`cloud-itonami-isic-2022`'s own `paintmfg.store`), this build ships
  ...
  Decision 1, this vertical is self-contained (no external pesticide/
  agrochemical-manufacturing capability library, no jurisdiction-scoped
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/weight/active-ingredient-
                             concentration record. `:verified?` marks
    ...
    - `equipment`         -- a formulation/mixing (blending tank,
                             high-shear mixer) or filling/packaging-
                             line unit's own record. `:verified?`/
    ...
    - `maintenance`       -- a scheduled formulation/mixing/filling/
                             packaging-line maintenance-window DRAFT
                             against a piece of equipment
                             (`pesticidemfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound pesticide/agrochemical
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (pesticidemfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (pesticidemfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :herbicide :output-form :liquid
                :material "Glyphosate SL Herbicide Concentrate"
                :weight-kg 5000.0 :active-ingredient-pct 41.0
                :verified? true :registered? true
                :shipped-weight-kg 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :insecticide :output-form :liquid
                :material "Pyrethroid EC Insecticide"
                :weight-kg 8000.0 :active-ingredient-pct 10.0
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :fungicide :output-form :powder
                :material "Copper Oxychloride WP Fungicide"
                :weight-kg 2000.0 :active-ingredient-pct 50.0
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"blender-001" {:id "blender-001" :kind :blending-tank
                   :verified? true :registered? true
                   :last-maintenance-date "2026-05-01"}
   "mixer-002" {:id "mixer-002" :kind :high-shear-mixer
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `pesticidemfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `pesticidemfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- two verified+registered batches within their own active-
  ingredient label ceiling (one with shipping headroom, one nearly
  fully shipped so a small new shipment blows through its own logged
  weight -- HARD hold), one UNVERIFIED/unregistered batch (blocks any
  shipment coordinated against it); one verified+registered blending-
  tank unit (schedulable for maintenance), one UNVERIFIED/unregistered
  high-shear-mixer unit (blocks any maintenance scheduling against
