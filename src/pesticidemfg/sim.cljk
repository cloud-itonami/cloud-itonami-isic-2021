(ns pesticidemfg.sim
  ...
  production weight, a proposal that tries to ACTUATE the formulation/
  mixing or filling/packaging line directly (permanently blocked, no
  override), a proposal that tries to DECIDE a pesticide registration
  or label approval directly (permanently blocked, no override), a
  ...
  an implausible active-ingredient concentration, and a
  production-batch patch whose declared active-ingredient
  concentration exceeds its product type's own regulatory label
  ceiling.
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-type :herbicide :last-assessed "2026-07-14"}}
                       coordinator))

    (println "== schedule-maintenance mnt-1 on blender-001 (verified, registered, blending-tank -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on blender-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on mixer-002 (UNVERIFIED/unregistered high-shear mixer -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on blender-001 with :actuate-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with :decide-registration? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-type :unobtainium-chemical}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible active-ingredient concentration -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:active-ingredient-pct -5.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with active-ingredient concentration exceeding the herbicide label ceiling -> HARD hold ==")
