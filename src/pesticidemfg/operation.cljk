(ns pesticidemfg.operation
  "PesticideOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (PesticideAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Pesticide &
  Agrochemical Plant Operations Governor (:govern) and the rollout
  ...
  (:require [langgraph.graph :as g]
            ...
            [pesticidemfg.advisor :as advisor]
            [pesticidemfg.governor :as governor]
            [pesticidemfg.phase :as phase]
...
(defn build
  "Compiles a PesticideOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `pesticidemfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PesticideAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Pesticide & Agrochemical Plant Operations Governor -- independent censor.
      (g/add-node :govern
